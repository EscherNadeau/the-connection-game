...
class SearchService {
  ...
  processResults(results, options = {}) {
    log('info', `Processing ${results.length} raw results`)
  ...
  applyFilters(results, options) {
    ...
    log('info', `Applied filters: ${results.length} → ${filtered.length} results`)
    return filtered
  }
  ...
  updateFilters(newFilters) {
    ...
    log('info', 'Filters updated:', this.currentFilters)
  }
  ...
  async getHint(query, options = {}) {
    log('info', `Getting hint for "${query}"`)
  ...
  async testSearch() {
    ...
    try {
      // Test 1: Basic search
      log('info', '🧪 Test 1: Basic search for "jack black"')
      ...
      log('info', `✅ Basic search: Found ${searchResult.results.length} results`)

      // Test 2: Search with actor filter
      log('info', '🧪 Test 2: Search with actor filter')
      ...
      log('info', `✅ Actor filter search: Found ${actorResult.results.length} results`)

      // Test 3: Get hint
      log('info', '🧪 Test 3: Get hint for "tom hanks"')
      ...
      log('info', `✅ Hint search: Found ${hintResult.results.length} results`)

      // Test 4: Check search state
      const state = this.getSearchState()
      log('info', '🧪 Test 4: Search state check')
      log('info', 'Search state:', state)

      log('info', '🎉 All Search Service tests completed!')
      ...
      log('error', 'Search service test failed:', error.message)
